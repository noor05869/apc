...
import OpenAccount from "../../SubComponent/OpenAccount";
  ...
  generateChart = async (type, subtype) => {
    ...
    if (type == "kibor") {
      ...
      if (response.length > 0) {
        let resultObj = response.find(
          (o) => o.tenor.toUpperCase() === timePeriod
        );
        ...
        if (resultObj) {
          ...
          bidAmount =
            +resultObj.bid * this.state.timeMarks[this.state.time].scaledValue;
        }
      }
